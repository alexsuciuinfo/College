...
joc_minimax(st(MIN,Tablou,N,MAX)):-
juc_opus(MAX,MIN),
muteu(Tablou,Tablou1,MIN),nl,
joc_minimax(st(MAX,Tablou1,N,MAX)).
 
joc_minimax(st(MAX,Tablou,N,MAX)):-
juc_opus(MAX,MIN),
minimax(st(MAX,Tablou,N,MAX),st(MIN,Tablou1,_,MAX),_),nl,
afis_tablou(Tablou1),write('-----------'),nl,nl,
joc_minimax(st(MIN,Tablou1,N,MAX)).
